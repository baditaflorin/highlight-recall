...
The app may request static JavaScript/CSS assets from GitHub Pages. Optional AI features may download browser model assets from the package defaults used by `@huggingface/transformers` or `@mlc-ai/web-llm` after the user clicks the relevant AI action. None of these requests include the content of imported documents, highlights, notes, or review history.
